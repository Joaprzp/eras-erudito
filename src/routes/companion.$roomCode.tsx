...
import { CompanionLobbyView, SoundControl } from '../components/companion'
  ...
  return (
    <main className={isBoardView ? 'min-h-screen bg-ink px-3 py-3 text-paper sm:px-6 sm:py-5 lg:px-8' : 'grid min-h-screen place-items-center bg-ink px-6 text-center text-paper'}>
      <div className={isBoardView ? 'mx-auto w-full max-w-[110rem]' : 'max-w-xl'}>
        ...
        {lobby ? (
          lobby.phase === 'active' ? (
            <GameBoard lobby={lobby} soundEnabled={soundEnabled} onToggleSound={toggleSound} playSound={playSound} />
          ) : lobby.phase === 'finished' ? (
            <FinishedBoard lobby={lobby} soundEnabled={soundEnabled} onToggleSound={toggleSound} />
          ) : (
            <CompanionLobbyView lobby={lobby} joinUrl={joinUrl} />
          )
        ) : null}
      </div>
    </main>
...
const JUDGE_MESSAGES = ['El juez delibera', 'El juez analiza', 'El juez evalúa', 'El juez pondera', 'El juez considera', 'El juez reflexiona']

function JudgePanel({ judge }: { judge: Judge }) {
  const [judgeMessageIndex, setJudgeMessageIndex] = useState(0)

  useEffect(() => {
    if (judge.status !== 'deliberating') return
    const interval = window.setInterval(() => {
      setJudgeMessageIndex((prev) => (prev + 1) % JUDGE_MESSAGES.length)
    }, 3000)
  ...
  if (judge.status === 'deliberating') {
    if (judge.thinking) {
      return <div aria-live="polite" className="mt-5 rounded-2xl border-2 border-mint/40 bg-mint/20 px-4 py-3">
        <p className="text-[0.6rem] font-black uppercase tracking-[0.18em] text-ink/55">Juez</p>
        <p className="mt-1.5 text-sm font-semibold leading-snug text-ink/85 whitespace-pre-wrap">{judge.thinking}</p>
      </div>
    }
    return <div aria-live="polite" className="mt-5 rounded-2xl border-2 border-dashed border-ink/25 bg-mint/25 px-4 py-3">
      <p className="font-display text-2xl tracking-[-0.04em]">{JUDGE_MESSAGES[judgeMessageIndex]}…</p>
    </div>
  }
